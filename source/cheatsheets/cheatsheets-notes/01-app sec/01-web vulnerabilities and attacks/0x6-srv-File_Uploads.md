...
2.  **Bypass Frontend**: Disable JS or intercept requests.
3.  **Fuzz Content-Type**: Change `Content-Type` header to `image/jpeg` or `image/png`.
4.  **Magic Bytes**: Add valid file signatures (e.g., `GIF89a`) to the beginning of the file.
5.  **Upload Web Shell**: Attempt to upload a shell and access it.
