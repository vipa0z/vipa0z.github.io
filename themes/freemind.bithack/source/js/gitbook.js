...
const GitBook = {
      ...
      if (item.type === "dir") {
        ...
        li.innerHTML = `
          <div class="nav-folder" data-path="${item.path}">
            <i class="fa fa-chevron-right folder-arrow"></i>
            ${iconHtml}
            <span>${item.name}</span>
          </div>
        `;
        ...
        if (item.children && item.children.length > 0) {
          ...
          // Changed: Removed ${item.isOpen ? 'open' : ''} to force collapsed by default
  ...
  renderQuickLinks(structure) {
    ...
    this.elements.quickLinksGrid.innerHTML = topLevelFolders
      .map(
        (folder) => `
    ...
    // Add click handlers
    this.elements.quickLinksGrid
      .querySelectorAll(".quick-link-card")
      .forEach((card) => {
    ...
    if (path === "cheatsheets/index.md") {
      if (this.initialContent) {
        ...
        const quickLinksGrid =
          this.elements.contentBody.querySelector("#quick-links-grid");
        if (quickLinksGrid) {
  ...
  updateBreadcrumb(path) {
    ...
    let html =
      '<a href="/cheatsheets/" class="breadcrumb-link" data-path="cheatsheets/index.md">Home</a>';
  ...
  updateNavButtons(path) {
    const index = this.state.flattenedNav.findIndex(
      (item) => item.path === path,
    );
  ...
  highlightActiveNav(path) {
    ...
    const activeLink = this.elements.sidebarNav.querySelector(
      `[data-path="${path}"]`,
    );
    if (activeLink) {
      ...
      while (parent) {
        ...
        const arrow =
          parent.previousElementSibling?.querySelector(".folder-arrow");
  ...
  handleSearch(query) {
    const searchResults =
      this.elements.sidebarNav.querySelector(".search-results");

    if (!query.trim()) {
      // Remove any existing search results and show normal nav
      if (searchResults) searchResults.remove();
      this.elements.sidebarNav
    ...
    const results = this.state.searchData.filter(
      (item) =>
